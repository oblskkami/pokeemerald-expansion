...
static const struct Usm_MenuItem sUsmMenuItems[USM_ICO_COUNT] = {
    [USM_ICO_POKEDEX] = USM_MENU_ITEM(Pokedex, "图鉴"),
    [USM_ICO_PARTY]   = USM_MENU_ITEM(Party, "宝可梦"),
    [USM_ICO_BAG]     = USM_MENU_ITEM(Bag, "包包"),
    [USM_ICO_POKENAV] = USM_MENU_ITEM(Pokenav, "宝可导航"),
    [USM_ICO_DEXNAV]  = USM_MENU_ITEM(Dexnav, "图鉴导航"),
    [USM_ICO_TRAINER] = USM_MENU_ITEM(Trainer, "训练家卡"),
    [USM_ICO_SAVE]    = USM_MENU_ITEM(Save, "记录"),
    [USM_ICO_REST]    = USM_MENU_ITEM(Save, "重置"),
    [USM_ICO_OPTIONS] = USM_MENU_ITEM(Options, "设置"),
    [USM_ICO_DEBUG]   = USM_MENU_ITEM(Debug),
    [USM_ICO_RETIRE]  = USM_MENU_ITEM(Retire, "撤退"),
};
...
static void Usm_PrintButtonHints()
{
    ...
    const u8* text = COMPOUND_STRING("{SELECT_BUTTON} 移动");
